...
function Holiday() {
  ...
  const handleCheckboxChange = async (id: number, currentStatus: number) => {
    ...
    try {
      const response = await fetch('/api/admin/holidays/statusUpdate', {
        method: 'PUT',
        headers: {
  ...
  const deleteHoliday = async (id: number, title: string, date: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`/api/admin/holidays/delete?id=${id}`, {
            method: 'DELETE',
          });
  ...
  const handleAddHoliday = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/holidays/add', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
