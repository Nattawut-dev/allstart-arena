...
function BuffetReserved() {
    ...
    const status = (status: number) => {
        return <td className='' style={{ backgroundColor: status === 0 ? '#FDCE4E' : status === 1 ? '#d1e7dd' : '#eccccf' }}>
            {status === 0 ? 'กำลังตรวจสอบ' : status === 1 ? 'ชำระแล้ว' : 'สลิปไม่ถูกต้อง'}
        </td>
    }
    const shuttle_cock_status = (status: string) => {
        return <td className='' style={{ backgroundColor: status === '' ? '#eccccf' : '#d1e7dd'}}>
            {status === '' ? 'ยังไม่ชำระ' : 'ชำระแล้ว'}
        </td>
    }
    ...
    const slipCheck = (id: number, slip_url: string, price: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch('/api/admin/buffet/update_status', {
                        method: 'PUT',
                        headers: {
    ...
    const saveEdit = () => {
        ...
        fetch('/api/admin/buffet/updateData', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const Delete = (id: number, name: string, nickname: string) => {
        Swal.fire({
            title: `ยืนยันลบการจองของ ${name}(${nickname}) ?`,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch(`/api/admin/buffet/updateData?id=${id}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json'
                        },
    ...
    return (
        <>
                        ...
                        {currentItems.map((buffet, index) => (
                            <tr key={index + indexOfFirstItem + 1}>
                                ...
                                <td className='d-flex justify-content-around'>
                                    <Button className='btn btn-sm' onClick={() => slipCheck(buffet.id, buffet.paymentSlip, buffet.price)} >Slip Check </Button>
            ...
            <Modal show={editBuffet !== null} onHide={() => setEditBuffet(null)}>
                    ...
                    {editBuffet && (
                        <Form>
                            <Form.Group controlId="formName">
                                ...
                                <Form.Control type="text" value={editBuffet.name} onChange={(e) => setEditBuffet({ ...editBuffet, name: e.target.value })} />
                            </Form.Group>
                            ...
                            <Form.Group controlId="formPrice">
                                ...
                                <Form.Control type="number" value={editBuffet.price} onChange={(e) => setEditBuffet({ ...editBuffet, price: parseInt(e.target.value) })} />
                            </Form.Group>
                            <Form.Group controlId="formPrice">
                                <Form.Label>ชำระค่าลูก ? <span className='text-danger'>*เว้นว่างสำหรับยังไม่ชำระ</span></Form.Label>
                                <Form.Control type="text" value={editBuffet.paymethod_shuttlecock} onChange={(e) => setEditBuffet({ ...editBuffet, paymethod_shuttlecock: e.target.value })} />
                            </Form.Group>
