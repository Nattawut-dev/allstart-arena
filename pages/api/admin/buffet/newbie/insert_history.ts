...
if (req.method === 'POST') {
    ...
    try {
        ...
        if (court == '') { court = '-' }
        let queries = `
            INSERT INTO history_buffet_newbie (
        ...
        console.error('Error inserting history_buffet_newbie:', error);
        res.status(500).json({ error: 'Error inserting history_buffet_newbie' });
    } finally {
        connection.release();
    }
}
