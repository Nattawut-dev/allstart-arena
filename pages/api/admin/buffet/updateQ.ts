...
export default async function insertData(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'PUT') {
        ...
        try {
            ...
            if (
                ...
                for (const item of destinationItems) {
                    ...
                    await connection.query(
                        `
                        INSERT INTO buffet_shuttlecocks (buffet_id, shuttlecock_type_id, quantity)
                        VALUES (?, ?, ?)
            ...
            console.error('Error updating buffet data:', error);
            res.status(500).json({ error: 'Error updating buffet data' });
        } finally {
            connection.release();
        }
    } else {
