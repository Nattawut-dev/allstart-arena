...
interface Props {
    allListtournament: Listtournament[];
}
...
export const getServerSideProps: GetServerSideProps<Props> = async () => {
    ...
    return {
        props: {
            allListtournament: Listdata.results,
        },
    };
};
const DetailPage = ({ allListtournament }: Props) => {
    ...
    const { tournament, hand_level } = router.query;
    const fetchListtournament = async () => {
        setSelected(true)
        const res = await fetch(`/api/tournament/listtournament?id=${tournament}`);
        ...
        const handLevel = await response.json()
    ...
    useEffect(() => {
        if (tournament != undefined && hand_level != undefined) {
            fetchListtournament();
        }
    }, [tournament, hand_level]);
    ...
    return (
        <>
            ...
            {handlevel.length == 0 && allListtournament.length > 0 && tournament === undefined && (
                <div className={styles.tournamentSelection}>
                    <h2 className='my-2'>โปรดเลือกงานแข่งเพื่อเข้าดูผู้สมัคร</h2>
                    <div className={styles.radioContainer}>
                        {allListtournament.map((tournament, index) => (
            ...
            {selected && tournament != undefined && hand_level != undefined && (
                <div>
                    ...
                    <div className={styles.container}>
                        ...
                        <Button className='btn btn-danger my-1' style={{ position: 'absolute' }} onClick={backToggle}>
                            {'ย้อนกลับ'}
                        </Button>
                    </div>
                    ...
                    >
                        ...
                        <Modal.Body>
                            ...
                            <div className={styles.status}>
                                ...
                                <h6>ยอดชำระ <span style={{ color: 'red' }}> {selectHandlevel?.price?.toLocaleString()} </span> บาท</h6>
